...
object Vec2 {  
	...
	*/
	def random(r:Random = null) = new Vec2(randomNormal(r), randomNormal(r))
...
class Vec2(var x:Float, var y:Float) extends Vec {
	...
	*/
	...
	final def distance(x:Float, y:Float):Float =
		sqrt(distanceSquared(x,y)).toFloat
	...
	* @see <a href="http://en.wikipedia.org/wiki/Slerp">spherical linear
	* interpolation</a>
	*/
